...
struct DietChatView: View {
    ...
    private func rowView(_ row: ChatRow) -> some View {
        ...
        case .pending(let pending, let badge):
            ChatBubble(
                ...
                onDismiss: pending.failed && !pending.isRetryable ? { vm.dismiss(pending.id) } : nil
            )
    ...
    private var inputRow: some View {
        HStack(spacing: 8) {
            ...
            Button {
                // **접수된 것을 확인하고 나서 비운다.** 먼저 비우면, 그 사이 첫 장 교체가
                // 시작돼 전송이 거절될 때 쓴 문장만 사라진다. 전송은 뷰모델이 직접 건다 —
                // 여기서 `Task`로 넘기면 접수와 전송 사이가 창이 된다.
                if vm.accept(draft) {
                    draft = ""
                }
            } label: {
