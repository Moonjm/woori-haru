...
extension VehicleTheme {
    ...
    /// 카드 안 조각별 색(예: 「한 번에 얼마나」 도넛). **다섯이 확실히 갈려야 한다** —
    /// `accent` 투명도만 나눈 예전 방식은 다섯 조각이 거의 한 색으로 보였다.
    /// 색상 자체를 벌리고, 색맹을 고려해 명도도 함께 벌렸다(밝기가 고르게 섞이도록).
    /// `accent`(#2dd4bf)를 첫 자리에 둬 앱 톤과 이어지게 한다.
    ///
    /// **`warning`·`danger`를 그대로 쓰지 않는다** — 이 앱에서 노랑·빨강은 이미 「주의」·
    /// 「위험」 뜻을 가져서, 카테고리 색으로 재사용하면 그 뜻이 흐려진다. 대신 주황·분홍처럼
    /// 톤은 가깝되 다른 색을 새로 골랐다.
    static let chartCategories: [Color] = [
        accent,                                       // #2dd4bf 청록(기존 강조색)
        Color(red: 0.220, green: 0.741, blue: 0.973),  // #38bdf8 하늘색
        Color(red: 0.769, green: 0.710, blue: 0.992),  // #c4b5fd 보라
        Color(red: 0.925, green: 0.282, blue: 0.600),  // #ec4899 분홍
        Color(red: 0.984, green: 0.573, blue: 0.235),  // #fb923c 주황
    ]
