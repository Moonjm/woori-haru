...
struct StatsChargeSection: View {
    ...
    /// 헤더는 내용과 함께만 선다. 이 섹션은 창이 둘이라 게이트도 둘이다 — 월별 차트
    /// 셋은 기간 칩을 따르는 `monthly`에서, 도넛은 전 기간 집계에서 온다. 둘 중
    /// 하나만 살아도 헤더는 선다.
    ///
    ...
    /// 같은 규칙: 아무것도 안 골랐으면 가장 큰 칸을 기본으로 보여 준다.
    ...
    /// `HeatmapGrid`를 그대로 넘기면 맞는다 — `isoWeekdayLabel`(1=월요일)을 쓰는
    /// 다른 요일 분포 카드들과 헷갈리지 않는다.
