...
/// 「배터리」 섹션(열화 추세)을 묶는다.
///
/// `VehicleStatsViewModel`은 `/tesla/insights` 하나만 본다. 배터리 열화 추세(`healthViewModel`)와
/// 급속/완속 도넛(`totalsViewModel`)은 전 기간 집계라 기간 칩과 무관하다.
///
/// 기간 칩은 화면 맨 위 하나다. 충전 탭의 월 스와이프는 여기 걸지 않는다.
struct VehicleStatsTab: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 12) {
                ...
                // 기간 칩과 무관하다 — 역대 최고와 평균 둘 다 전 기간 값이라 `months`
                // 범위를 따르지 않는다. 「이 기간에 주행이 없다」는 이유로 감추면
                // 짧은 기간을 보는 사람에게 전 기간 평균까지 사라진다.
                //
                // `hasLoadedInsights`로 응답 전인지만 가린다 — 안 그러면 로딩 스피너
                // 위에 「—」 셋짜리 카드가 먼저 선다.
                ...
                // 섹션 순서는 고정이다: 주행 → 충전 → 주차 → 배터리 → 위치 → 기록.
                // `content`(거리 분포·히트맵·온도별 전비, 주행 없음 안내)는 「주행」
                // 섹션의 나머지 카드라 `StatsDriveSection` 바로 뒤에 둔다.
                //
                // 헤더는 `StatsDriveSection`과 `content`의 게이트(각각 hasDriveMonths·
                // hasDrives, 합치지 않는 이유는 아래 content 참고) 중 하나만 있어도 서야
                // 해서 `showsDriveSection`(OR)로 한 번만 판단한다.
    ...
    @ViewBuilder private var content: some View {
        ...
        if viewModel.isLoading && viewModel.insights == nil {
            ...
            // `hasDrives`(거리 버킷, `end_date` 기준)와 `hasDriveMonths`(월별 값,
            // `start_date` 기준)는 서버 소스가 달라 자정을 걸친 주행에서 어긋날 수
            // 있다 — 하나로 합치지 않는다. 둘 다 거짓일 때만 안내를 띄워야 주행
            // 카드는 있는데 「기록 없음」 안내도 뜨는 모순을 막는다.
    ...
    ///
    /// 헤더는 내용과 함께만 선다(바로 위 「🔌 충전」 카드와 구분하려고) —
