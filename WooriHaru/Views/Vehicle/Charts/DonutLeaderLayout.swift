...
enum DonutLeaderLayout {
    ...
    /// 도넛 중심을 원점으로 하는 네 점 — 조각 바깥 가장자리 → 방사형 토막 끝 → 꺾임점 → 라벨.
    /// `stubPoint`가 `edgePoint`와 같은 각도로 반지름만 키운 점이라, 위·아래 조각도
    /// 부챗살처럼 퍼져 나간 뒤에야 가로로 꺾여 서로 겹치지 않는다.
    ...
    ///   - stubLength: 방사형 토막의 길이(`radius`에 더해진다). 이 구간은 각도를 유지한다.
    ///   - bendX: 꺾임점의 중심에서 가로 거리(오른쪽/왼쪽은 함수가 부호를 맞춘다).
    ///   - labelX: 라벨이 앉는 점의 중심에서 가로 거리.
    /// - Returns: `slices`와 같은 개수·순서의 배치. 조각이 없으면 빈 배열.
    static func place(_ slices: [Slice], radius: CGFloat, stubLength: CGFloat,
                       bendX: CGFloat, labelX: CGFloat) -> [Placement] {
        guard !slices.isEmpty else { return [] }

        var cursor: CGFloat = 0
        let edged: [(index: Int, slice: Slice, side: Side, edge: CGPoint, stub: CGPoint)] =
            slices.enumerated().map { index, slice in
                ...
                let stubRadius = radius + stubLength
                let stub = CGPoint(x: stubRadius * cos(standardRad), y: -stubRadius * sin(standardRad))
                return (index, slice, side, edge, stub)
            }
        ...
        for side in [Side.right, Side.left] {
            ...
            // 정렬·간격 기준을 `stub.y`로 잡는다 — 꺾임이 방사형 토막 끝에서 시작하기 때문이다.
            let group = edged.filter { $0.side == side }.sorted { $0.stub.y < $1.stub.y }
