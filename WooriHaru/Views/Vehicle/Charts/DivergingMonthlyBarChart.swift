...
/// 음수를 0으로 자르는데, 그 가드가 곧 나머지 스물다섯 장(거리·시간·건수·금액·비율 — 전부
/// 음수가 원리적으로 불가능하다)의 계약이다. 부호가 실제로 뜻을 갖는 화면은 지금 #21
/// 「월별 대기 중 소모」 하나뿐이다. 기존 원형에 발산 모드를 심으면 그 분기 하나가 스물다섯
/// 장의 회귀 위험을 함께 짊어져야 한다 — "카드가 아니라 그림만 그린다"는 이 저장소의 원형
/// 규칙 안에서, 부호를 쓰는 화면은 자기 원형을 따로 갖는 편이 더 싸게 먹힌다. 기하 계산
