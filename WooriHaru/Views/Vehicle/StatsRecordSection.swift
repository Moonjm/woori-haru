...
/// 실측으로 0.2km 주행이 8.2배 효율로 1등을 먹는다).
///
/// 하나라도 있으면 섹션을 그린다. 개별 타일은 자기 레코드가 nil이면 「—」만 보이고
/// 다른 두 타일은 그대로 값을 낸다. 셋 다 없을 때만 헤더까지 감춘다(`showsRecords`).
///
/// `driveId`는 화면에 쓰지 않는다 — 지금 앱에 주행 상세 화면이 없다.
struct StatsRecordSection: View {
    ...
    private var header: some View {
        HStack(alignment: .firstTextBaseline) {
            ...
            // 조회한다(범위마다 1등이 바뀌면 기록이 아니다). 안 그러면 주행 없는
            // 기간을 골랐을 때 「기록 없음」 안내 바로 아래에 몇 년 전 기록 타일이
            // 떠서 어느 기간 이야기인지 헷갈린다.
            Text("전 기간")
                .font(.caption)
                .fontWeight(.bold)
    ...
    /// 1.745… → "1.7배" — 「정격 대비 174%」보다 「1등을 먹었다」는 느낌에 더 가깝다.
