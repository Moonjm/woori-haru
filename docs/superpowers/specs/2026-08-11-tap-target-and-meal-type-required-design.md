...
탭 제스처가 걸려 있지 않은 카드에서는 `contentShape`이 아무 일도 하지 않는다. 식단 끼니
행에는 이미 `GlassCard` 바깥에 같은 목적의 `.contentShape(Rectangle())` 우회로가 있었으므로,
실제로 새로 눌리게 되는 곳은 `SwimRecordListView` 한 곳뿐이다. 식단 쪽은 그 우회로를
걷어내 공용 컴포넌트로 일원화하는 것이다 — 사각 모서리로 남아 있던 히트 영역이 카드
모서리와 맞는 모양으로 바뀐다.
