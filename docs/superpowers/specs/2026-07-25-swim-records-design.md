...
## 자동 세트와 구간 — 시간 기준이 다르다

실제 기록(2026-07-25, 50m 레인 40랩 2000m)을 진단해 보니 랩 사이 공백이 두 종류로 뚜렷하게
갈렸다. 벽 찍고 턴하는 공백은 1~7초, 세트 사이 휴식은 1분 이상이었다. 이 공백을 20초 기준으로
끊으면 피트니스 앱의 자동 세트가 그대로 재현된다 — 휴식 시간이 초 단위로 일치했다.

그래서 시간을 재는 기준이 다른 두 가지를 나란히 보여준다.

| | 자동 세트 | 구간 |
| --- | --- | --- |
| 묶는 기준 | 쉬는 구간으로 나눔 (거리 제각각) | 50m·100m 고정 |
| 시간 | 첫 랩 시작 ~ 마지막 랩 종료 (**턴 포함**) | 랩 소요 시간의 합 (**턴 제외**) |
| 쓸모 | 실제 기록·목표 관리 | 순수 추진 속도 비교 |

구간 시간을 자동 세트처럼 전체 구간 시간으로 바꾸면 안 된다. 구간 경계 안쪽에 휴식이 들어가는
경우(예: 50m 세트 두 개가 한 100m 구간에 묶일 때) 휴식이 수영 시간으로 잡혀 속도를 왜곡한다.

피트니스는 **첫 세트를 운동 시작(0:00)부터** 잰다. 실측에서 첫 랩은 +0:44에 시작했고, 그
44초(물에 들어가기까지의 시간)가 첫 세트 수영 시간에 그대로 들어가 페이스가 2:43/100m로
왜곡됐다. 우리는 첫 랩 시작부터 재므로 첫 세트만 피트니스보다 짧게 나오며, 이쪽이 정확하다.
