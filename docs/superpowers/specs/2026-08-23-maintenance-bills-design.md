...
서버에 이미 답이 있다. `POST /maintenance/recognitions`가 고지서 사진 한 장을 받아 **항목·금액·사용량·납기일을 통째로 읽어 준다.** 저장은 하지 않고 결과만 주므로, 배차표와 똑같이 **사람이 눈으로 검수한 뒤 확정**하는 구조를 그대로 쓸 수 있다. `GET /maintenance/trends`는 기본 13개월을 주는데, 13이라는 숫자는 **전년 동월이 범위에 들어오라고** 고른 값이다 — 난방비처럼 계절을 타는 항목은 전월이 아니라 전년 동월과 견줘야 뜻이 있다.
...
- **알림을 넣지 않는다.** 납기일이 데이터에 있지만 이번엔 표시만 한다.
...
| `chargedAmount` | `Decimal` | 부과액 |
| `discountTotal` | `Decimal` | 할인 합계 |
| `unpaidAmount` | `Decimal` | 미납액 |
| `unpaidLateFee` | `Decimal` | 미납 연체료 |
| `dueAmount` | `Decimal` | **청구액 — 실제로 내는 돈** |
...
**추이에는 `dueAmount`가 없다.** 서버가 `chargedAmount`(부과액)만 준다. 목록·상세는 청구액을, 통계는 부과액을 쓴다 — **다른 숫자다.** 통계 화면에 「부과액 기준」이라고 적어 둔다. 이 차이를 안 적으면 목록의 8월 숫자와 통계 막대의 8월 숫자가 달라 보인다.
...
- `2026년 8월` — 큰 글씨
- 청구액(`dueAmount`) — 강조색, `monospacedDigit`
...
- 청구액 히어로 + 연월 + 세대(동/호/면적)
- 금액 요약 타일 2×2 — 부과액 · 할인 · 미납 · 연체료
...
`GET /maintenance/trends?months=13`. **부과액 기준**이라고 화면에 적는다.
