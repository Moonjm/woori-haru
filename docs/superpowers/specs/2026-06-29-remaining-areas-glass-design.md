...
- **버튼**: 파랑(blue500) CTA만 `appGlassProminentButton()`로. 회색(slate700)·주황(orange300) 버튼은 **solid 유지**.
...
- 파랑 CTA → `appGlassProminentButton()`. 회색·주황 버튼은 solid 유지.
...
- **ProfileView**: 루트 `ScrollView` → `glassScreenBackground()`. 폼 필드 전체를 **GlassCard**로 묶는다. 입력칸(흰+stroke)·아이디 readonly(slate100)·성별 토글·DatePicker는 유지. "저장"(slate700, 회색)은 **solid 유지**.
- **CategoriesView**: 루트 `Color.slate50`(14행) → `glassScreenBackground()`. 생성 폼 흰 카드(87행) → `glassEffect`. 목록 컨테이너 `.background(.white)`(159행) → `glassEffect` 패널. 행(slate50)·입력칸·ACTIVE 칩·드래그 핸들 유지. "추가하기"(orange300)는 solid 유지, 편집 저장(blue500) → glass.
- **AdminView**: 루트 `VStack` → `glassScreenBackground()`. `adminCard` 흰+shadow(63행) → `glassEffect`(탭 가능한 네비 카드). 아이콘 배경은 유지.
- **UserManagementView**: 루트 `Color.slate50`(90행) → `glassScreenBackground()`. 생성 폼 카드(47행)·목록 카드(84행) → `glassEffect`. 행(slate50)·입력칸·권한 칩/메뉴 유지. "사용자 추가"(slate700)는 solid 유지, 편집 저장(blue500) → glass.
...
4. **버튼 색감** — 회색/주황 버튼은 그대로(요청). 글래스 카드 위 solid 버튼이 무겁게 보이면 시각 QA에서 조정 후보.
