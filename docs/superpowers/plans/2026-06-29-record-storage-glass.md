# Record 영역 Glass 롤아웃 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Record 영역의 시트/카드/배경에 Foundation Liquid Glass를 적용하고, 캘린더에서 보류했던 `RecordSheetView` 글래스를 완성한다.
...
- **촘촘한 리스트 행은 가독성 유지**: 행 통째 글래스화 금지(기록 행 텍스트/이모지·진행바).
...
- **Spec 커버리지**: RecordSheetView(보류분) → Task 1; RecordFormView → Task 2; RecordListView/OvereatSelector → Task 3; 검증/리스크 → Task 4.
